...
include('scripts/class.upload.php');
...
} else if ($action == 'cgallery') {

    // ---------- IMAGES ON CREATE GALLERY ----------
    ...
    foreach ($_FILES['my_field'] as $k => $l) {
        foreach ($l as $i => $v) {
            ...
            $files[$i][$k] = $v;
                ...
                $gname = $_POST['gname'];
                $pname = str_replace(' ', '_', strtolower($gname));
                $phpname = $pname . ".php";
                $fwfolder = $_SERVER['DOCUMENT_ROOT'] . '/galleries/' . $phpname;
                $folder = 'galleries/' . $phpname;
                $fields1 = array('page' => $folder, 'private' => 0, 'title' => $gname, 'gpage' => 1); //column_name=>entry
                $db->insert('pages', $fields1);
                
                $fh = fopen($fwfolder, 'w');
                $fw = fwrite($fh, "<?php 
            include('gallery_template.php');
        ?>");
                if ( $fh == FALSE OR $fw == FALSE )
{
     echo "something went wrong in writing";
} else {
    echo "writing should have occurred";
} 
                fclose($fh);
    ...
    foreach ($files as $file) {
        ...
        if ($handle->uploaded) {
            ...
            if ($handle->processed) {
                // everything was fine !
                echo '<p class="result">';
                echo '  <b>File(s) uploaded with success</b><br />';
                echo '  File(s): <a href="' . $dir_pics . '/' . $handle->file_dst_name . '">' . $handle->file_dst_name . '</a>';
                $ilink = $dir_pics . '/' . $handle->file_dst_name;
                $fields = array('ilink' => $ilink, 'gname' => $gname); //column_name=>entry
                $db->insert('gallery', $fields);
                echo '   (' . round(filesize($handle->file_dst_pathname) / 256) / 4 . 'KB)';
                echo '</p>';
            } else {
                // one error occured
                echo '<p class="result">';
                echo '  <b>File(s) not uploaded to the wanted location</b><br />';
                echo '  Error: ' . $handle->error . '';
                echo '</p>';
            }
        } else {
            ...
            // i.e. the server didn't receive the file
            echo '<p class="result">';
            echo '  <b>File(s) not uploaded on the server</b><br />';
            echo '  Error: ' . $handle->error . '';
            echo '</p>';
        }
    }
} else if ($action == 'addgallery') {
